...
- Section cards have REQUIREMENTS (what Lance owes) and DELIVERABLES (what Pete ships) as state machines with clear status pills. Sections 2–7 are deliverables-only.
  REQUIREMENT LIFECYCLE (Lance's side): pending → submitted (Lance uploads his file to the shared drop folder — https://drive.google.com/drive/folders/1c0Mcfw6L28OOCjI8qToFET6MB63NRtS0 — then clicks Submit and pastes a Drive URL) → approved (Pete reviews) OR rejected with a note (Lance resubmits). In Section 1 there is a prominent yellow "Drop your files here" link right above the requirement rows so Lance can get to the folder in one click.
