...
- Section cards have REQUIREMENTS (what Lance owes) and DELIVERABLES (what Pete ships) as state machines with clear status pills. Sections 2–7 are deliverables-only.
  REQUIREMENT LIFECYCLE (Lance's side): pending → submitted (Lance uploads his file into the CATEGORY-specific Drive sub-folder for that requirement, then clicks Submit and pastes the Drive URL) → approved (Pete reviews) OR rejected with a note (Lance resubmits). Every pending requirement row has its own "Upload here → [Category]" link so Lance knows exactly which folder his file belongs in — brand assets go to the brand folder, credentials go to the credentials folder, artist roster goes to the artist folder, etc. The category routing map lives in ClientPortal.tsx under REQ_DROP_FOLDERS.
